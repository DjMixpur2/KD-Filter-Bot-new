...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    #     btn.insert(0, [
    #         InlineKeyboardButton("sᴛᴀʀᴛ ʙᴏᴛ", url=f"https://telegram.me/{temp.U_NAME}"),
    #         InlineKeyboardButton("", callback_data=f"send_fall#{pre}#{key}#{offset}")
    #     ])
    try:
        await query.edit_message_reply_markup(
            reply_markup=InlineKeyboardMarkup(btn)
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        btn2 = [[
                 ...
                 InlineKeyboardButton("", url="telegram.me/netflixvilla_india")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            reply_markup = InlineKeyboardMarkup(btn)
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            ...
            dlt = await message.reply_sticker('CAACAgIAAxkBAAJt7ma-8eOmO7o6CpPzLN-qkr66H7djAALtAQACsPvbB_uWvHNc8ni7NQQ', reply_markup=InlineKeyboardMarkup(btn))
            search = search.lower()
            find = search.split(" ")
            search = ""
    ...
    #     btn.insert(0, [
    #         InlineKeyboardButton("Sᴛᴀʀᴛ Bᴏᴛ", url=f"https://telegram.me/{temp.U_NAME}"),
    #         InlineKeyboardButton("", callback_data=f"send_fall#{pre}#{message.chat.id}-{message.id}#{0}")
    #     ])
        btn.insert(0, [
            InlineKeyboardButton("Sᴛᴀʀᴛ Bᴏᴛ", url=f"https://telegram.me/{temp.U_NAME}"),
            InlineKeyboardButton("", callback_data=f"sendfiles#{key}")
